...
class OrinParserTests(unittest.TestCase):
    ...
    def test_parser_normalizes_password_reset_source_with_expected_semantic_context(self):
        ...
        expected_uncertainty_line = next(
            index for index, line in enumerate(source_path.read_text(encoding="utf-8").splitlines(), 1) if line.strip() == "uncertainty: rate-limit"
        )
        uncertainty = next(
            item for item in model.document["objects"]
            if item["id"] == "account.password-reset/uncertainty/rate-limit"
        )

        self.assertEqual(model.document["module"]["imports"], ["account-store", "email-provider"])
        self.assertEqual(uncertainty["source"]["line"], expected_uncertainty_line)
